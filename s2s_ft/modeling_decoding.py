...
class BertEncoder(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, output_all_encoded_layers=True, prev_embedding=None,
                prev_encoded_layers=None, mask_qkv=None, seg_ids=None, key_history=None, value_history=None, rel_pos=None):
        ...
        if (prev_embedding is not None) and (prev_encoded_layers is not None):
            ...
            for i, layer_module in enumerate(self.layer):
                hidden_states = layer_module(
                    hidden_states, attention_mask, history_states=history_states, mask_qkv=mask_qkv, seg_ids=seg_ids, rel_pos=rel_pos)
...
class BertForSeq2SeqDecoder(PreTrainedBertModel):
    ...
    def __init__(self, config, mask_word_id=0, num_labels=2, num_rel=0,
                 ...
                 pos_shift=False, repetition_penalty=1.0, stop_word_ids=[]):
        ...
        while next_pos < output_length:
            ...
            if len(total_scores) == 0:
                ...
                if self.repetition_penalty != 0.0:
                    ...
                    kk_scores = kk_scores.squeeze(1)
                    kk_ids = kk_ids.squeeze(1)
                    for bbm_id in range(len(partial_seqs)):
                        bbm_pseq = partial_seqs[bbm_id]
                        # print(bbm_pseq, repetition_penalty)
                        bbm_word_ids = kk_ids[bbm_id].tolist()
                        for bm_id in range(len(bbm_word_ids)):
                            if bbm_word_ids[bm_id] in bbm_pseq and bbm_word_ids[bm_id] not in self.stop_word_ids :
                                kk_scores[bbm_id][bm_id] *= self.repetition_penalty
                    kk_scores = kk_scores.unsqueeze(1)
                    kk_ids = kk_ids.unsqueeze(1)
