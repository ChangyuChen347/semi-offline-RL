...
class BertForSequenceToSequenceWithPseudoMask(BertForSequenceToSequence):
    ...
    def forward(
        ...
        def loss_mask_and_normalize(loss, mask):   #todo
            mask = mask.type_as(loss)
            loss = loss * mask
            denominator = torch.sum(mask) + 1e-5
            return (loss / denominator).sum(), loss.sum(-1) / (mask.sum(-1) + 1e-5)

        prediction_scores_masked = self.cls(pseudo_sequence_output)

        probs = torch.softmax(prediction_scores_masked.float(), dim=-1)
        log_probs = torch.log(probs)

        _, y_b = torch.max(log_probs, dim=-1)

        # sample y_s
        multi_dist = Categorical(probs)
        y_s = multi_dist.sample()
        sample_log_probs = torch.gather(log_probs, dim=-1, index=torch.unsqueeze(y_s, -1)).squeeze()
        ...
        pseudo_lm_loss, ce_loss_re = loss_mask_and_normalize(masked_lm_loss.float(), target_mask)


        return pseudo_lm_loss, y_s, y_b, sample_log_probs, target_mask, ce_loss_re
