...
flags.DEFINE_string(
    ...
    'documentation).')
flags.DEFINE_enum(
    'input_format', None, ['wikisplit', 'discofuse', 'ads'],
...
def _compute_lcs(source, target):
  ...
  """
  table, bt_table = _lcs_table(source, target)
  return _backtrack(bt_table, source, target, len(source), len(target))
...
def _lcs_table(source, target):
  ...
  bt_table = [[0] * (cols + 1) for _ in range(rows + 1)]
    ...
    for j in range(1, cols + 1):
      if source[i - 1] == target[j - 1]:
        ...
        if lcs_table[i-1][j] >= lcs_table[i][j-1]: 
          ...
          bt_table[i][j] =3 
  return lcs_table, bt_table

def _backtrack(bt_table, source, target, i, j): 
  lcs = []
  while bt_table[i][j] != 0: 
    if bt_table[i][j] == 1: 
      lcs.append(target[j-1])
      i -= 1 
      j -= 1
    elif bt_table[i][j] == 2: 
      i -= 1 
...
def _get_added_phrases(source: Text, target: Text) -> Sequence[Text]:
  ...
  e.g. src : a b, tgt : a a a b, then lcs is a b, but src should be able to generate tgt 
