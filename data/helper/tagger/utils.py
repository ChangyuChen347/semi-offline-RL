...
def yield_sources_and_targets(
  ...
  elif input_format == "ads": 
    yield_example_fn = _yield_ads
  else:
...
def _yield_ads(input_file): 
  with open(input_file) as f:
    for line in f:
      target, source = line.rstrip('\n').split('\t')
      yield [source], target

def read_label_map(path, is_log: str=False):
  ...
  with open(path) as f:
    if path.endswith('.json'):
      ...
      for tag_id, tag in enumerate(f):
        ...
        if tag:
          if is_log: 
            label_map[keep_tag] = len(label_map)
            label_map[del_tag] = len(label_map)
          else: 
