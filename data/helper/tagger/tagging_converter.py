...
import data.helper.tagger.tagging as tagging
import data.helper.tagger.utils as utils
...
class TaggingConverter(object):
  ...
  def _compute_tags_fixed_order(self, source_tokens, target_tokens):
    ...
    while target_token_idx < len(target_tokens):
      # print("src:", source_tokens)
      # print("tgt:", target_tokens)
    ...
    if target_token_idx >= len(target_tokens):
      return tags
    # raise ValueError("return empty list")
    return []
  ...
  def _compute_single_tag(
    ...
    added_phrase = ''
    # case where current src token not match current tgt token (pos t)
    # and need to use added phrases(size k) to match tgt tokens (t -> t+k)
