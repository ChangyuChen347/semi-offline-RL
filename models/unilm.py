...
class BertPreTrainedModel_bak(nn.Module):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, state_dict=None, cache_dir=None,
                        warmup_checkpoint=None, hub_path=None, from_tf=False, no_segment=False, rel_pos_type=0, max_rel_pos=128, rel_pos_bins=32, fast_qkv=False, hidden_dropout_prob=0.1, attention_probs_dropout_prob=0.1, task_dropout_prob=0.1, remove_task_specifical_layers=False, keep_cls=True, *inputs, **kwargs):
        """
        ...
        config.task_dropout_prob = config.classifier_dropout if config.classifier_dropout is not None else config.hidden_dropout_prob
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        extended_attention_mask = (
            1 - extended_attention_mask).type(torch.bool)
...
@register_model("seq2label_unilm","Bert")
...
class BertForTqna(BertPreTrainedModel):
    def __init__(self, config, num_labels):
        super(BertForTqna, self).__init__(config)
...
class BertForQuestionAnswering(BertPreTrainedModel):
    ...
    def re_norm(self, ffn_scale, self_attention_scale):
        def perform(dense, scale, name):
            norm = dense.weight.data.norm(2).item()
            k = norm / scale
            if scale > 0:
                logger.info(
                    "Re-norm {}, norm = {},  k = {}".format(name, norm, k))
                dense.weight.data.div_(k)
                dense.bias.data.div_(k)

        for layer_id, layer in enumerate(self.bert.encoder.layer):
            perform(layer.attention.output.dense, self_attention_scale,
                    "layer_{}_self".format(layer_id))
            perform(layer.output.dense, ffn_scale,
                    "layer_{}_ffn".format(layer_id))
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, end_positions=None):
        ...
        if start_positions is not None and end_positions is not None:
            ...
            return start_logits, end_logits, _


class BertForQuestionAnsweringForReCoRD(BertPreTrainedModel):
    def __init__(self, config):
        super(BertForQuestionAnsweringForReCoRD, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                entity_positions=None, is_answer_entity=None, entities_mask=None, query_position=None):
        sequence_output, _ = self.bert(input_ids, token_type_ids, attention_mask,
                                       output_all_encoded_layers=False)

        def gather_seq_out_by_pos_average(seq, pos):
            # pos/mask: (batch, num_pair, max_token_num)
            batch_size, num_pair, max_token_num = pos.size()
            ...
            mask = 1.0 - pos.eq(0).type_as(pos_vec)
            pos_vec_masked_sum = (
        ...
        entities = gather_seq_out_by_pos_average(
            sequence_output, entity_positions)
        query_token = gather_seq_out_by_pos_average(
            sequence_output, query_position)
        logits = self.qa_outputs(
            torch.cat((entities, query_token.expand_as(entities)), dim=-1))

        if is_answer_entity is not None:
            loss = self.loss_func(logits.view(-1, 2),
                                  is_answer_entity.view(-1))
            mask_var = entities_mask.type_as(loss)
            masked_loss = torch.mul(loss, mask_var.view(-1)).sum()
            return masked_loss / mask_var.sum()
        else:
            return logits
...
class BertForPreTrainingMLM(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, is_random_next=None,
        ...
        if masked_lm_labels is not None and is_random_next is not None:
            ...
            num_correct_tokens = torch.mul(predict_index.eq(
                masked_lm_labels).long(), masked_weights).sum()
            ...
            next_sentence_loss = self.loss_mask_and_normalize(
                next_sentence_loss, is_next_mask)
            _, predict_relationships = seq_relationship_score.max(-1)
            num_correct_relationship = (predict_relationships.eq(is_random_next).type_as(
                next_sentence_loss) * is_next_mask.type_as(next_sentence_loss)).sum()

            rets = [masked_lm_loss, next_sentence_loss]
            infos = [num_correct_tokens, num_mask_tokens, num_correct_relationship,
                     is_next_mask.sum().type_as(next_sentence_loss)]

            if match_labels is not None:
                val_output = gather_seq_out_by_pos_average(
                    sequence_output, val_indexs)  # , mask=val_index_mask)
                col_output = gather_seq_out_by_pos_average(
                    sequence_output, col_indexs)  # , mask=col_index_mask)
                ...
                match_score = self.match_head(
                    col_output, col_index_mask, val_output, val_index_mask, match_label_mask)

                match_loss = self.crit_mask_lm(match_score, match_labels)
                match_loss = self.loss_mask_and_normalize(
                    match_loss, match_label_mask)
                rets.append(match_loss)

                _, predict_index = match_score.max(1)
                num_correct_match = torch.mul(predict_index.eq(
                    match_labels).long(), match_label_mask).sum()
