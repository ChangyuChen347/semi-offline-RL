...
from s2s_ft.utils import load_and_cache_examples, build_special_token_dict
...
def detokenize(tk_list, unused2token_dict=None):
    r_list = []
    for tk in tk_list:
        if tk.startswith('##') and len(r_list) > 0:
            r_list[-1] = r_list[-1] + tk[2:]
        else:
            # substitue if it's [unused]
            r_list.append(unused2token_dict[tk] if unused2token_dict and tk in unused2token_dict else tk)
    return r_list
...
def main():
    ...
    parser.add_argument("--special_token_ids", default=None, type=str, required=False,
                        help="Extra words to not tokenize on. Use comma to separate multi words.")
    parser.add_argument('--is_tsv', action='store_true',
                        help="Input file is tsv file. Assume json file if not set.")
    ...
    # replace special tokens to [unused] tokens
    token2unused, unused2token, unused_tokens = build_special_token_dict(args.special_token_ids)
    logger.info("special_token_dict: " + str(token2unused))

    tokenizer = TOKENIZER_CLASSES[args.model_type].from_pretrained(
        args.tokenizer_name, do_lower_case=args.do_lower_case, 
        do_basic_tokenize=True, never_split=unused_tokens, 
    ...
    for model_recover_path in glob.glob(args.model_path):
        ...
        model = BertForSeq2SeqDecoder.from_pretrained(
            ...
            max_position_embeddings=args.max_seq_length, pos_shift=args.pos_shift, repetition_penalty=args.repetition_penalty
        )
        ...
        to_pred = load_and_cache_examples(
            ...
            special_token_dict=token2unused, is_tsv=args.is_tsv)
                ...
                with torch.no_grad():
                    ...
                    for i in range(len(buf)):
                        ...
                        if args.model_type == "roberta" or args.model_type == "xlm-roberta":
                            ...
                            output_sequence = ' '.join(detokenize(output_tokens, unused2token))
                        if '\n' in output_sequence:
